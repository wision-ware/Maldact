...
/// <summary>
/// Orchestrates the gradient descent training lifecycle for the TimeSeriesCnn model.
/// Ensures safe native memory boundaries, responsive async cancellation, and LOH-optimized parameter serialization.
/// </summary>
public class CnnTrainingLoop : ITrainingLoop
{
    private readonly TimeSeriesCnn _model;

    /// <summary>
    /// Initializes a new instance of the training loop.
    /// </summary>
    /// <param name="model">The target neural network. Lifecycle disposal must be managed by the caller.</param>
    /// <exception cref="ArgumentNullException">Thrown if the provided model is null.</exception>
    public CnnTrainingLoop(TimeSeriesCnn model)
    {
        ...
        CancellationToken ct = default)
    {
        return await Task.Run(() =>
        {
            ...
            // allocate unique physical checkpoint file to dodge unmanaged stream closure exceptions
            ...
            try
            {
                for (int epoch = 0; epoch < MaxEpochs; epoch++)
                {
                    ...
                    foreach (var batch in dataLoader.GenerateBatches())
                    {
                        ...
                        // trap implicit c++ allocations generated during the iteration block
                ...
                // load the optimal weights into memory exactly once 
